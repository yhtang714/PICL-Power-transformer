...
@dataclass
class PICLDataset:
    data: torch.Tensor            # [N, n_vars] = fault one-hot + gas features
    gas_values: torch.Tensor      # [N, G] the gas block fed to the SCM
    labels: torch.Tensor          # [N]
    source: torch.Tensor          # [N]  synthetic source id
    miss_mask: torch.Tensor       # [N, n_vars]  True = missing
    is_synthetic: torch.Tensor    # [N]  generated by Module D?
    log_ppm: Optional[torch.Tensor] = None   # [N, G]  log1p(ppm), used by classifier head


def _stratified_round_robin(labels, n_sources, seed):
    # Round-robin assignment, stratified by class, so every source gets
    # roughly equal class proportions.
...
def _normalise_proportions(gas_raw, miss_mask_gas):
    # Row-wise normalise to proportions. Denominator uses observed gases only.
    # NaNs are preserved so the SCM log-likelihood can marginalise them.
...
# Cache log1p z-score stats so impute_training_set can invert the transform
# back to ppm space when refreshing log_ppm at originally-missing positions.
_LAST_LOG_MU = None
_LAST_LOG_SD = None
...
def load_picl_datasets(cfg: PICLConfig):
    ...
    if feature_mode == "log1p_z":
        ...
        log_sd = (np.nanstd(tr_log, axis=0) + 1e-8).astype(np.float32)
    ...
    for split_name in ("train", "cal", "test"):
        ...
        # SCM data row: [fault one-hot | gas]
        ...
        # Use a different seed offset for each split so cal/test get their
        # own source distribution (rather than all falling on source 0).
        if split_name == "train":
...
def concat_datasets(a: PICLDataset, b: PICLDataset) -> PICLDataset:
    # Fall back to zeros if either side has no log_ppm, to keep concat working.
    def _lp(d):
